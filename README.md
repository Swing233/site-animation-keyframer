# site-animation-keyframer — Agent Skill
...
每次迭代后都会推送新版本（commit 号见 git log）。如需要完全自动的更新，可用系统计划任务（cron / launchd / 客户端自带自动化）每日执行上面的 `git pull`。
...
- **时间轴**：播放头 scrub、帧吸附、Shift+拖动精细微调、Alt+滚轮锚点缩放、音频波形对齐
- **视角**：摄像机/自由视角切换、视觉中心拾取、自由视角一键同步为关键帧、参考网格、导出画幅安全框
